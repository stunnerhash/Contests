...
vector<vector<int>> dp(498 + 1, vector<int>(40000 + 1, -1));
...
int coinchange(vector<int>& a, int v, int n)
{
    ...
    if (a[n - 1] <= v) 
        return dp[n][v] = (coinchange(a, v - a[n - 1], n) + coinchange(a, v, n - 1))%mod;
    else return dp[n][v] = coinchange(a, v, n - 1);
}
void Solve()
{
	int v = 40000;
