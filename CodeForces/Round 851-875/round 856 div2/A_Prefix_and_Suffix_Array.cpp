...
void solve() {
	...
	string aa = a[0], bb = a[1];
	if(aa[0] == bb[mx-1]){
		if(aa.substr(1,mx-1)  == bb.substr(0,mx-1) and ispal (aa.substr(1,mx-1))) yes
	}
	if(bb[0] == aa[mx-1]){
		if(bb.substr(1,mx-1)  == aa.substr(0,mx-1) and ispal(bb.substr(1,mx-1))) yes
	}
	no	
}
