...
void solve() {
	int n; cin>>n;
	if(n%10 == 9){
		vi a; while(n) a.pb(n%10), n/=10;
		int x = 0, y = 0;
		bool flag = 0;
		for(int i = 0;i<a.size();i++) {
			if(a[i]%2 == 0) x += power(10,i)*(a[i]/2),y += power(10,i)*(a[i]/2);
			else{
				if(flag) x += power(10,i)*(a[i]/2), y += power(10,i)*(a[i]/2+1);
				else x += power(10,i)*(a[i]/2+1), y += power(10,i)*(a[i]/2);
				flag = 1- flag ;
			}
		}
		cout<<x<<' '<<y<<endl;
