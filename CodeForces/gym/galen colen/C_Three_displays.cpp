...
{
	int n; cin>>n;
	vector<vl> dp(n,vl(3,1e9));
	vl s(n); loop cin>>s[i];
	loop cin>>dp[i][0];
	loop lop(k,i) if(s[k]<s[i]) dp[i][1] = min(dp[i][1],dp[i][0]+dp[k][0]);
	loop lop(k,i) if(s[k]<s[i]) dp[i][2] = min(dp[i][2],dp[i][0]+dp[k][1]);
	
	int ans = 1e9; loop ans = min(dp[i][2],ans);
