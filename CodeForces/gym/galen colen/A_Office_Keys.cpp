...
{
	int n,k,p; cin>>n>>k>>p;
	vi a(n); loop 	  cin>>a[i]; sort(all(a));
	vi b(k); lop(i,k) cin>>b[i]; sort(all(b));
	int ans = 1e18;
	lop(id,k-n+1){
		int sum = 0;
		loop{
			if ((a[i]<=b[id+i])  and (b[id+i]<=p) ) sum = max(sum,abs(a[i] - p));
			else sum = max(sum,abs(a[i]-b[id+i]) + abs(b[id+i]-p));
		}
		ans = min(sum,ans);
	}
	cout<<ans<<endl;
