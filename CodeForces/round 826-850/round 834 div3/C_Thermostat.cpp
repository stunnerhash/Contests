...
void solve()
{
	...
	int a,b; cin>>a>>b;
	if(a == b) return (0);
	if(ok(l,r) == 0) return (-1); 
	if(abs(a-b) >=x) return (1);
	int btr = ok(b,r), btl = ok(b,l);
	int atl = ok(a,l), atr = ok(a,r);
	int ans = -1;
	if(atl and btr) ans = 3;
	if(atr and btl) ans = 3;
	if(atl and btl) ans = 2;
	if(atr and btr) ans = 2;
	return(ans);
}
