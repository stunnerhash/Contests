...
void solve()
{
	...
	if(n>3 or (n == 3 and id != 1) or n == 1) returnn(a[id]*n);
	if(n == 2) returnn(max(a[0]+a[1] , 2*abs(a[0]-a[1])));
	int sum = 0; loop sum += a[i]; 
	loop lop(k,n) sum = max(sum,3*abs(a[i]-a[k]));
	cout<<max(sum,3*max(a[0],a[2]))<<endl;
}
