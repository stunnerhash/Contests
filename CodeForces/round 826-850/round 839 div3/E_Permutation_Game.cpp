...
void solve()
{
	...
	vi b = a, c = a; sortall(b); sort(all(c),greater<int>());
	int asc = 0, dec = 0, ext = 0;
	loop{
		if (a[i] != b[i] and a[i] != c[i]) ext++;
		else if (a[i] != b[i]) asc++;
		else if (a[i] != c[i]) dec++;
	}
	if (asc + ext <= dec) returnn("First")
	else if(asc > dec + ext) returnn("Second")
	else returnn("Tie");
}
