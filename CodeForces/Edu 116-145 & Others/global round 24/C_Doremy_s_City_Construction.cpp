...
void solve()
{
	int n; cin>>n; 
	vl a(n); loop cin>>a[i]; 
	map<int,int> mp; loop mp[a[i]]++;
	vl pre; for(auto i:mp) pre.pb(i.ss);
	if(pre.size() == 1) return(pre.back()/2);
	for(int i =1 ;i<pre.size();i++) pre[i] += pre[i-1];
	int sum = pre.back(), ans = 0;
	lop(i,pre.size()) ans = max(ans,pre[i]*(sum-pre[i]));
	cout<<ans<<endl;
}
